...
async function tryAllEndpoints(query) {
  ...
  for (const endpoint of ENDPOINTS) {
    try {
      ...
      return { ok: true, result: attachNearestAddress(parseOverpass(data)) };
    } catch (e) {
      lastErr = e;
...
// ── Address gap-filling ────────────────────────────────────────────────
// OSM doesn't tag every building with an address — plenty of building
// footprints exist with a shape but no addr:housenumber/addr:street. Rather
// than reverse-geocoding each one (which would reintroduce the same
// Nominatim rate-limit problem we just fixed, one extra call per
// unaddressed building), associate it with the NEAREST addressed building
// from the same Overpass response we already have. This is free (no extra
// API calls) and naturally groups clusters of unaddressed structures under
// whichever tagged address they're actually next to.
const NEAREST_ADDRESS_RADIUS_M = 150;
...
function attachNearestAddress(structures) {
  const addressed = structures.filter(s => s.address);
  if (addressed.length === 0) return structures;
  for (const s of structures) {
    if (s.address) continue;
    let best = null, bestDist = Infinity;
    for (const a of addressed) {
      const d = haversineMeters(s.lat, s.lon, a.lat, a.lon);
      if (d < bestDist) { bestDist = d; best = a; }
    }
    if (best && bestDist <= NEAREST_ADDRESS_RADIUS_M) {
      s.near_address = best.address;
      s.near_address_distance_m = Math.round(bestDist);
    }
  }
  return structures;
}

module.exports = { fetchStructuresNear, MAX_RADIUS_M, attachNearestAddress };
